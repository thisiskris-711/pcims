...
try {
    ...
    case 'sales':
        ...
        $orderBy = $allowedSorts[$sortColumn] ?? 's.created_at';
        ...
        if ($export === 'csv') {
            $stmt = $db->prepare("
                SELECT s.invoice_no, COALESCE(d.name, 'Walk-in Customer') as customer_name, s.subtotal, s.discount, s.tax, s.total,
                       ...
                       (SELECT COUNT(*) FROM sale_items WHERE sale_id = s.id) as item_count, s.dealer_id, s.notes
                $baseQuery
        ...
        jsonResponse(['data' => $data, 'summary' => $summary, 'page' => $page, 'total_pages' => $totalPages ?? 1, 'total' => $count ?? count($data)]);
        break;
